...
import { stopBackendChild } from './backend-child.ts'
import { hiddenWindowsChildOptions } from './windows-child-options.ts'
...
test('hiddenWindowsChildOptions merges windowsHide alongside other options on Windows', () => {
  assert.deepEqual(
    hiddenWindowsChildOptions({ encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] }, true),
